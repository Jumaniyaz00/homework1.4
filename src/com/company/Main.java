...
public class Main {
    ...
    public static int[] heroesHealths = {250, 250, 250};                   // NEW
    public static int doctorhelth = 150;                                   // NEW
    ...
    public static void heroesHit() {
        for (int i = 0; i < heroesDamages.length; i++) {
            if (!heroesAttackTypes[i].equals(bossDefenceType)) { // heroesAttackTypes[i] != bossDefenceType
                Random r = new Random();
                int coefficient = r.nextInt(1) + 2; //0,1,2,3,4
                ...
                if (doctorhelth > 0) {
                    heroesHealths[i] = heroesHealths[i] + doctorHelp;
            }
